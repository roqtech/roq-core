...
export const messageFormat = (): Format =>
  winston.format((info) => {
    ...
    if (info.type === LoggingTypeEnum.incomingRequest) {
      info.message = `Incoming request ${info.request?.url} on ${info.service}${info.caller ? ` from ${info.caller}` : ''}`;
    }
    ...
    if (info.type === LoggingTypeEnum.outgoingRequestResponse) {
      info.message = `Response of outgoing response on ${info.service}${info.request?.url ? ` to ${info.request?.url}` : ''}`;
    }
    ...
    if (info.type === LoggingTypeEnum.outgoingRequestError) {
      info.message = `Error response of request on ${info.service}${info.request?.url ? ` to ${info.request?.url}` : ''}`;
    }
